...
function spawnLine(zPos) {
    var lanes = [road.laneXLeft, road.laneXMid, 
        road.laneXRight];
    var elementsCount = Math.round(Math.random() * lanes.length);
    var obstaclesCount = 0;
    for(var i = 0; i < elementsCount;i++){
        var r = Math.random();
        var genObstacle = r > script.coinGenChance;
        if(genObstacle){
            if(obstaclesCount >= 2) return;
            obstaclesCount++;
        }
        var prefab = genObstacle ? script.obstaclePrefab 
            : script.coinPrefab;
        if (!prefab) return;
