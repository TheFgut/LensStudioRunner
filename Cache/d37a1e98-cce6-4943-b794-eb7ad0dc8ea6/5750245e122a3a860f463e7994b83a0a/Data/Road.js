...
function init() {
    ...
    for (var i = 0; i < script.segmentCount; i++) {
        ...
        if (roadPartScript && roadPartScript.api && roadPartScript.api.length) {
            segmentLength = roadPartScript.api.length;
        } else {
...
updateEvent.bind(function(eventData) {
    ...
    if (!script.gameManager || !script.gameManager.api || !script.gameManager.api.getSpeed) return;

    var speed = script.gameManager.api.getSpeed(); // Берем скорость из GameManager
    ...
    if (firstPos.z + segmentLength < -segmentLength) {
        ...
        if (first.script && first.script.api && first.script.api.reset) {
            first.script.api.reset();
        }
