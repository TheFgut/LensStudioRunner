...
// @input float roadInitialZOffset = 100
...
function init() {
    if (!script.roadPartPrefab) {
        print("Road.Error No link to road part prefab");
        return;
    }

    if (!script.gameManager) {
        print("Road.Error No link to game manager");
        return;
    }
...
function gameStateChanged(newState){
    ...
    else if(newState === "Loose") stopGeneration = true;
    else if (newState === "Run") stopGeneration = false;
}

function reset(){
    var zPos = script.roadInitialZOffset;
    for (var i = 0; i < segments.length; i++) {
        segments[i].script.reset();
        segments[i].script.getTransform()
            .setLocalPosition(new vec3(0, 0, zPos));
        zPos += segmentLength;
    }
}
